...
impl  Page for HomePage {
    fn draw_page(&self) -> Result<()>{
        ...
        //TODO: print out epics using get_column_string(), also make epics are sorted by id
        println!();
        println!();
        Ok(())
    }
    ...
    fn handle_input(&self, input: &str) -> Result<Option<Action>> {
        todo!() //match against user input and return the corresponding action. If the user input was invalid return None.
    }
}

pub struct EpicDetial {
    pub epic_id: u32,
    pub db: Rc<JiraDatabase>
}

impl Page for EpicDetial {
    fn draw_page(&self) -> Result<()> {
...
mod tests {
    ...
    #[test]
    fn handle_input_should_return_correct_actions(){
        let db = Rc::new(JiraDatabase {database:Box::new(MockDB::new())});
        ...
        let junk_input_with_valid_prefix = "j983f2j";
        ...
        assert_eq!(page.handle_input(&valid_epic_id).unwrap(), Some(Action::NavigateToEpicDetail {epic_id}));
    ...
    mod epic_detail_page {
        ...
        fn draw_page_should_not_throw_error(){
            ...
            let page = EpicDetial{epic_id, db};
            assert_eq!(page.draw_page().is_ok(), true);
        }
        #[test]
        ...
        #[test]
        fn draw_page_should_error_for_invalid_epic_id(){
            let db = Rc::new(JiraDatabase {database:Box::new(MockDB::new())});

            let page = EpicDetial{epic_id: 999, db};
            assert_eq!(page.handle_input("").is_err(), true);
        }
        ...
        fn handle_input_should_return_the_correct_actions(){
            ...
            assert_eq!(page.handle_input(&story_id.to_string()).unwrap(), Some(Action::NavigateToStoryDetail {epic_id: 1, story_id: 1}));
        ...
        mod story_detail_page {
            ...
            fn handle_input_should_return_correct_actions(){
                ...
                assert_eq!(page.handle_input(p).unwrap(), Some(Action::NavigateToPreviousPage));
                assert_eq!(page.handle_input(u).unwrap(), Some(Action::UpdateStoryStatus {story_id: 1}));
